...
export function InitialColumn({ rows, setOverColumn, gates }) {
  const drop = useDrop(
    () => {
      // Check for special method only when drop actually happens
      const specialMethod = gates.isSpecialMethod();
      if (specialMethod === ADDCOLUMNRIGHTAT) {
        gates.moveGate(0, -1);
      }
    },
    -1,
    (c) => {
      // Check for special method only when drag enters
      const specialMethod = gates.isSpecialMethod();
      if (specialMethod === ADDCOLUMNRIGHTAT) {
        setOverColumn(c);
      }
