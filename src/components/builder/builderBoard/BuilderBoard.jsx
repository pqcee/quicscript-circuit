 ...
 * @param {boolean} props.updateSelect - Flag for updating selection area.
 * @param {Function} props.setUpdateSelect - Function to update selection area.
 * @param {number} props.overColumn - Flag to determine if a modifier is over a column.
 * @param {Function} props.setOverColumn- Function to set which column a certain gate for modifier is currently over.
...
export function BuilderBoard({
  ...
  const controlledArray = useMemo(() => {
    if (!getGateSlots || getGateSlots.length === 0) {
      return [];
    }
    ...
    >
      ...
      {getGateSlots.map((row, qubit) => (
        <RowHolder key={qubit}>
          ...
          <DeleteButtonHolder style={GateSize}>
            <DeleteButton onClick={() => gates.deleteQubit(qubit)}>
              X
            </DeleteButton>
          </DeleteButtonHolder>
        </RowHolder>
